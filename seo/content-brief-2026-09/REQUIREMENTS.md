...
| A4 / §10-A.4 | Remove repeated boilerplate; derive prose from page-specific evidence | Implemented in part. Sept 16 follow-up found the remaining duplication on the 10 `/compare/*` targets was the Verdict/FAQ generator, not prose gaps — `compareEditorial` covered pros/cons and "when to choose" but the Verdict boxes and FAQ section still rendered unconditionally from `comparison-content.ts` regardless. Extended with bespoke `verdict`/`faqs` per pair; local diagnostic dropped 20-31 points per page (55-65%→31-41%), real SiteLiner re-scan not yet run against it. The 7 send-money hubs and 11 companies/banks/IBAN targets are dominated by shared widget/nav-chrome text instead of prose and were left alone — see README's "§10-A step 7, continued". All 37 remain above the *local* threshold in the existing build (expected; that checker has never passed 0/37, see its own doc comment). |
