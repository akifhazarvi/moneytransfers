...
| A4 / §10-A.4 | Remove repeated boilerplate; derive prose from page-specific evidence | Implemented in part. Sept 16 follow-up found the remaining duplication on the 10 `/compare/*` targets was almost entirely three generator sources, not prose gaps — `compareEditorial` covered pros/cons and "when to choose" but the Verdict boxes, FAQ section and `keyDifferences` bullets still rendered unconditionally from `comparison-content.ts` regardless. All three replaced with bespoke per-pair content; local diagnostic dropped further each pass (55-65%→31-41%→26-36%). **4 of the 10 (wise-vs-remitly, ofx-vs-xe, paypal-vs-revolut, wise-vs-paypal) now pass the local checker outright — first time any brief target has** (0/37→4/37). Real SiteLiner re-scan not yet run against either pass. The 7 send-money hubs and 11 companies/banks/IBAN targets are dominated by shared widget/nav-chrome text instead of prose and were left alone — see README's "§10-A step 7, continued". The other 33 remain above the local threshold. |
...
| A7 / §10-A.7 | Fresh SiteLiner crawl: every processed page below 30% | Partial — real Premium crawl obtained (`siteliner-fresh-2026-09-16.csv`, 540 processed pages). 9/37 targets pass; 28 remain at or above 30%. Across all 37, 25 scores are lower and 12 higher; the expanded corpus means these are not controlled measurements of rewrite effects. Later commits still need post-deployment validation. The local diagnostic is a separate method, not a substitute for SiteLiner. |
