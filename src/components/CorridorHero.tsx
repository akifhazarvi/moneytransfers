...
export default function CorridorHero({
  ...
  return (
    <section className="bg-[var(--color-surface-dim)]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-4 sm:pt-6 pb-10 sm:pb-14">
        {/* Breadcrumb */}
        <nav className="flex items-center gap-2 text-xs text-[var(--color-on-surface-muted)] mb-4">
        ...
        {/* Soft navy gradient hero panel */}
        <div className="ws-hero px-6 sm:px-10 py-10 sm:py-14">
          {/* Title row */}
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8 lg:gap-12">
            <div className="max-w-2xl">
              ...
              {/* Meta line — single row, on the gradient */}
              <div className="flex flex-wrap items-center gap-x-3 gap-y-2 mt-6 text-sm text-white/70">
                <span className="inline-flex items-center gap-1.5">
                  <CircleFlag code={fromCurrencyCode} size={20} priority={true} />
                  <span className="font-medium tabular-nums text-white/90">{fromCurrency}</span>
                </span>
                ...
                {midRate > 0 && (
                  <>
                    ...
                    <span className="hidden sm:inline">
                      Mid-market <span className="font-medium text-white/90 tabular-nums">{midRate.toFixed(4)}</span>
                    </span>
                  </>
                )}
              </div>
          ...
          {best && bestName && (
            <div className="lg:min-w-[380px] lg:max-w-[420px] w-full">
              <div className="ws-card p-5 sm:p-6 relative overflow-hidden">
                ...
                <div className="grid grid-cols-2 gap-4 mb-5">
                  ...
                  <div>
                    <p className="text-[11px] font-medium text-[var(--color-on-surface-muted)] uppercase tracking-wider">Estimated receipt</p>
                ...
                {showSavings && (
                  <div className="bg-[var(--color-success-surface)] border border-[var(--color-success-surface-dim)] rounded-xl px-3.5 py-2.5 mb-4">
                    <p className="text-xs text-[var(--color-success-dark)]">
                      ...
                      <span className="text-[var(--color-on-surface-variant)]"> than the lowest estimate</span>
                      {markupPct > 0.05 && (
