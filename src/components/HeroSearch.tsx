  ...
  return (
    <div ref={wrapperRef} className={`relative ${compact ? "" : "max-w-xl mx-auto mt-4 sm:mt-6"}`}>
      ...
      >
        ...
        <button
          onClick={() => submit()}
          className={`shrink-0 ${compact ? "w-6 h-6" : "w-8 h-8 sm:w-9 sm:h-9"} rounded-full bg-[var(--color-cta)] hover:bg-[var(--color-cta-hover)] text-[var(--color-cta-text)] flex items-center justify-center transition-colors`}
          aria-label="Search"
        >
