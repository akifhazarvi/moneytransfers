 ...
 *
 * Presentation only: receives the selected rails without importing the dataset.
 */
...
export default function CryptoRailSectionView({
  from, to, rails, cashout,
}: CryptoRailSectionData) {
    ...
    >
      ...
      <div className="divide-y divide-[var(--color-outline)]/60">
        {rails.map((r) => {
          ...
          const btc = r.chains.some((c) => c.token === "BTC" || /lightning/i.test(c.name));
          return (
