...
export default function StatBox({ label, value, className = "", badge }: Props) {
  return (
    ...
    >
      ...
      <p className="text-md font-semibold text-[var(--color-on-surface)] leading-snug">{value}</p>
      {badge && (
        <span className="inline-block mt-1 text-2xs font-semibold text-[var(--color-success-dark)] bg-[var(--color-success-surface)] px-2 py-0.5 rounded-full">
          {badge}
        </span>
      )}
    </div>
