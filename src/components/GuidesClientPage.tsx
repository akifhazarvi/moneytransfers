...
export default function GuidesClientPage({ posts, categories, featured }: Props) {
  ...
  const showFeatured = activeCategory === "All" && !query.trim() && safePage === 1 && sort === "updated";
      ...
      <section ref={topRef} tabIndex={-1} id="guide-library" className="guide-library" aria-labelledby="guide-library-title">
        ...
        {pagePosts.length ? (
          ...
          <div className="guide-empty">
            ...
            <h3>Let’s try a different search</h3>
            <p>Try a provider like Wise, a country like India, or a topic like transfer fees.</p>
            <button type="button" onClick={() => { setQuery(""); setActiveCategory("All"); setPage(1); searchRef.current?.focus(); }}>Browse all guides <ArrowRight size={16} aria-hidden="true" /></button>
          </div>
        )}
