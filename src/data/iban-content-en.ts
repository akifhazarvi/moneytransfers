...
export const ibanContentEn: IbanContent = {
  ...
  romania: {
    ...
    intro:
      "Romania uses a 24-character IBAN and is a full SEPA member, enabling low-cost euro transfers to and from other SEPA countries. The National Bank of Romania (BNR) oversees the IBAN standard and the domestic SENT (electronic clearing) and ReGIS (RTGS) payment systems. While Romania's currency is the leu (RON), the IBAN works equally for RON and EUR accounts held at Romanian banks.",
    bullets: [
      "A Romanian IBAN starts with RO, two check digits, a four-character alphanumeric bank code (such as RNCB for BCR, BRDE for BRD, INGB for ING Romania, and BTRL for Banca Transilvania), and a 16-character account number. The four-letter bank identifier makes it easy to spot which institution holds the account.",
      "As a SEPA member, Romania participates in SEPA Credit Transfer (SCT) and SEPA Instant Credit Transfer (SCT Inst), meaning euro payments from eurozone countries are fast and inexpensive. However, RON transfers from outside Romania are processed through SWIFT and may carry correspondent banking fees.",
      "Romanians living abroad commonly send remittances home via specialist transfer services. When directing a transfer to a Romanian bank account, always provide the full 24-character RO IBAN. For RON-denominated payments, confirm whether the sending provider supports direct RON delivery or routes via EUR with a conversion at BNR's reference rate.",
    ],
  },
  czechia: {
