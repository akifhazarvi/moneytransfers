...
export const ibanContentEn: IbanContent = {
  editorial: {
    "bulgaria": {"title": "Check the Bulgarian account, not just the BG prefix", "intro": "A Bulgarian IBAN has 22 characters. The four-letter bank identifier within it is only one part of the receiving instruction: the rest identifies the account. Two accounts at the same institution therefore cannot be substituted for one another.", "bullets": ["Keep the letters and leading zeros when copying a BG account from the bank-issued record. Grouping spaces can be removed for an electronic form; account characters cannot.", "For a Bulgarian invoice, reconcile the beneficiary name and full IBAN with the supplier’s confirmed instructions. A successful format check does not establish that the supplier owns the account."]},
