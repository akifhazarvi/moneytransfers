...
export const corridorEditorial: Record<string, CorridorEditorial> = {
  // Written 2026-09-25. A ranking corridor (see ranking-corridors.ts) with no
  // comparable quote in our data, so it states what the data does and does
  // not show rather than naming a cheapest option.
  "switzerland-to-egypt": {
    theRoute: `Switzerland to Egypt is a thin route in our data. Our feeds carry one indicative CHF→EGP rate, from a currency broker that quotes on request, and no provider publishes a fee-inclusive franc-to-pound quote we can rank against another. What we can give you is the benchmark: at today's mid-market rate of {{MID_RATE:CHF:EGP}}, CHF 1,000 is worth {{MID_RECEIVE:CHF:EGP:1000}}. Any quote you are offered should be measured against that figure, and the gap is the provider's real price. At that rate each 1% of margin on CHF 1,000 costs the recipient about {{MID_RECEIVE:CHF:EGP:10}}; if your bank adds a CHF 25 wire fee, that is another {{MID_RECEIVE:CHF:EGP:25}} the recipient never sees. A quick test for any offer on CHF 1,000: if it pays less than {{MID_RECEIVE:CHF:EGP:970}}, you are paying more than 3% all-in.`,
    measuredRecord: `Because no two providers quote CHF→EGP at the same amount, this page cannot rank them. The Egypt routes we can compare are listed on our <a href="/send-money/send-money-to-egypt">Egypt page</a>, where several providers quote each sending currency.`,
    localRails: {
      heading: "How Egyptian pounds reach the recipient",
      body: `On the Egyptian side the payout choice sets the timing. Bank deposits land in an Egyptian bank account, mobile-wallet payouts reach a phone number, and cash pickup suits a recipient without either. Confirm with the recipient which of these they can actually use before you compare prices, because a provider that only pays to bank accounts is no use to someone who needs cash.`,
    },
    beforeYouSend: {
      heading: "Francs straight to pounds, or via a second currency",
      body: `From Switzerland you will usually fund in francs from a Swiss bank account. Ask whether a quote converts CHF directly into EGP or routes the money through euros or dollars first: each conversion carries its own margin, and a route through a second currency is where a bank's cost usually hides. Keep the quote and the transfer reference until the recipient confirms the amount received.`,
    },
  },
  ...
  "usa-to-india": {
    ...
    localRails: {     heading: "UPI, IMPS, NEFT and RTGS — what a US-funded transfer actually rides on",
      body: `On a US-funded transfer, India's rails are no longer the bottleneck. <a href="/send-money/send-money-to-india">UPI, IMPS, NEFT and RTGS</a> all run round the clock, so a payment sent on Saturday is not waiting for Monday on the Indian side. What adds time is the US side: ACH funding from a US bank account typically takes one to two business days to clear before the Indian leg starts, while a debit card funds instantly but usually carries a higher fee. If the arrival date matters, the US funding method is what to change. For larger sums, check whether your provider pays out over RTGS, which settles each payment individually rather than in NEFT's half-hourly batches (source: <a href="https://www.rbi.org.in/scripts/FS_FAQs.aspx?Id=65" target="_blank" rel="noopener noreferrer">RBI RTGS FAQ</a>).` },
    beforeYouSend: {
  ...
  "usa-to-philippines": {
    ...
    localRails: {    heading: "InstaPay, PESONet and the wallets — plus the US-funding lag",
      body: `A US sender's delay usually happens before the money reaches the Philippines. On the Philippine side, <a href="/send-money/send-money-to-philippines">InstaPay and PESONet</a> behave differently — InstaPay is real-time but capped per transaction, PESONet settles in batches — and GCash and Maya wallets ride InstaPay. On the US side, funding from a bank account via ACH typically needs a day or two to clear before the transfer starts, which is why a debit-card-funded transfer often reaches a GCash wallet first even when it costs a little more. For a larger amount above the InstaPay cap, ask the provider whether it splits the payment or routes it through PESONet, because that decides the arrival day.` },
    beforeYouSend: {
  ...
  "usa-to-pakistan": {
    ...
    localRails: {    heading: "Raast, IBFT and the wallets — and the US-funding lag",
      body: `For a US sender the Pakistani end is rarely the slow part: <a href="/send-money/send-money-to-pakistan">Raast and IBFT</a> credit a bank account within moments, and JazzCash and Easypaisa wallets do the same for recipients without one. The clock that matters starts in the US. ACH funding from a US bank account typically needs a day or two to clear before any money leaves, while a debit card funds at once but usually at a higher fee — so on a deadline, the funding method is the lever to pull. The other US-specific check is cost that no rate table shows: some Pakistani banks waive charges on inward remittances that qualify under the <strong>Pakistan Remittance Initiative</strong>, so ask the receiving bank before choosing a payout method.` },
    beforeYouSend: {
