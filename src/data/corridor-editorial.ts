...
export const corridorEditorial: Record<string, CorridorEditorial> = {
  "usa-to-india": {
    ...
    localRails: {     heading: "UPI, IMPS and NEFT — what each means for arrival time",
      body: `India runs several domestic rails and which one your provider uses decides when the money is usable, independently of how fast the provider says it is. <strong>UPI</strong> settles in seconds and runs 24/7 including weekends and holidays, addressed by a UPI ID rather than an account number. <strong>IMPS</strong> is also immediate and round-the-clock, addressed by account number plus IFSC. <strong>NEFT</strong> settles in half-hourly batches on working days, so a Friday-evening NEFT payment can sit until Monday. <strong>RTGS</strong> is for large values and settles in real time on working days. These are properties of the rail, published by NPCI and the Reserve Bank of India, not promises by any transfer company — a provider quoting "instant" is describing the leg it controls, which does not include the time your own funding takes to clear or how long verification runs on a first transfer. If the deadline matters, pick the payout rail first and the provider second.` },
    beforeYouSend: {
