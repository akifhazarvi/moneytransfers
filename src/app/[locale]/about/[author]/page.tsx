...
export default async function AuthorPage({ params }: Props) {
  ...
  const personSchema = {
    ...
    ...(author.photo && { image: `https://sendmoneycompare.com${author.photo}` }),
          ...
          <div className="max-w-3xl">
            <div className="flex items-center gap-5 mb-6">
              {author.photo ? (
                ...
                <div className="w-20 h-20 rounded-full bg-[var(--color-primary-surface)] flex items-center justify-center text-2xl font-medium text-[var(--color-primary)] shrink-0">
                  {author.initials}
                </div>
              )}
              <div>
                <h1 className="text-h3 md:text-4xl font-normal text-[var(--color-on-surface)] leading-tight">
                  {author.name}
                </h1>
                <p className="text-md text-[var(--color-primary)] mt-1">{author.role}</p>
                {author.linkedin && (
                  <a
                    href={author.linkedin}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-1.5 text-2sm text-[var(--color-on-surface-variant)] hover:text-[var(--color-primary)] mt-2"
                  >
      ...
      <section className="py-12">
        <Container>
          <div className="max-w-3xl space-y-10">
            {/* Bio */}
            <div>
              <h2 className="text-h4 font-normal text-[var(--color-on-surface)] mb-4">About</h2>
            ...
            {/* Expertise */}
            <div>
              <h2 className="text-h4 font-normal text-[var(--color-on-surface)] mb-4">Areas of expertise</h2>
              <div className="grid sm:grid-cols-2 gap-3">
                {author.expertise.map((item) => (
                  <div key={item} className="flex items-center gap-2 text-sm text-[var(--color-on-surface-variant)] bg-[var(--color-surface-dim)] rounded-lg p-3">
            ...
            {/* Credentials */}
            <div>
              <h2 className="text-h4 font-normal text-[var(--color-on-surface)] mb-4">Credentials</h2>
              <ul className="space-y-2">
                {author.credentials.map((item) => (
                  <li key={item} className="flex items-start gap-2 text-sm text-[var(--color-on-surface-variant)]">
            ...
            {authorPosts.length > 0 && (
              <div>
                ...
                {author.articlesWritten > 10 && (
                  <Link href="/guides" className="text-sm text-[var(--color-primary)] hover:underline mt-4 inline-block">
