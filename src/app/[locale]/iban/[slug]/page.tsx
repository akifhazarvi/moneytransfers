...
// indexable URL space of them. Verified 2026-09-03: /iban/zzz-bogus answered 200
...
// generateStaticParams (wiseCountries), so nothing that should render starts 404ing.
...
const ibanMetaOverrides: Record<string, { title: string; description: string; ogTitle?: string; ogDesc?: string }> = {
  ...
  poland: {
    title: "Polish IBAN: 28-Char Format + Example (2026)",
  ...
  ireland: {
    title: "Irish IBAN: 22-Char Format + Example (2026)",
  ...
  spain: {
    title: "Spanish IBAN: 24-Char Format + Example (2026)",
  ...
  italy: {
    title: "Italian IBAN: 27-Char Format + Example (2026)",
  ...
  belgium: {
    title: "Belgian IBAN: 16-Char Format + Example (2026)",
  ...
  "united-arab-emirates": {
    title: "UAE IBAN: 23-Char Format + Example (2026)",
  ...
  netherlands: {
    title: "Dutch IBAN: 18-Char Format + Example (2026)",
  ...
  romania: {
    title: "Romanian IBAN: 24-Char Format + Example (2026)",
  ...
  portugal: {
    title: "Portuguese IBAN: 25-Char Format + Example (2026)",
  ...
  finland: {
    title: "Finnish IBAN: 18-Char Format + Example (2026)",
  ...
  denmark: {
    title: "Danish IBAN: 18-Char Format + Example (2026)",
  ...
  hungary: {
    title: "Hungarian IBAN: 28-Char Format + Example (2026)",
  ...
  qatar: {
    title: "Qatar IBAN: 29-Char Format + Example (2026)",
  ...
  egypt: {
    title: "Egyptian IBAN: 29-Char Format + Example (2026)",
  ...
  luxembourg: {
    title: "Luxembourg IBAN: 20-Char Format + Example (2026)",
...
export default async function IbanCountryPage({ params }: Props) {
  ...
  // Each BBAN field shows the example IBAN's own segment. A bare "4 chars"
  // column read identically on every country sharing a layout, and the
  // lengths alone are already in the validation card below.
  let bbanOffset = 4;
  for (const field of country.bbanFields) {
    const label = bbanLabels[field.type] || field.label.replace(/_/g, " ");
    const segment = country.exampleIban.slice(bbanOffset, bbanOffset + field.length);
    bbanOffset += field.length;
    ibanParts.push({
      label: label.charAt(0).toUpperCase() + label.slice(1),
      value: segment || `${field.length} ${field.length === 1 ? "char" : "chars"}`,
      length: field.length,
      color: "var(--color-on-surface-variant)",
    });
  }

  // One set of answers for the visible FAQ and its JSON-LD. The template
  // answers these replaced ("on your bank statement, in your online banking
  // portal…", "processed quickly and at low cost through the SEPA network")
  // were word-for-word the same on every IBAN page and restated the structure
  // card; SiteLiner scored /iban/bulgaria 58% duplicate on 2026-09-26.
  const bankNames = country.banks.map((bank) => bank.name);
  const defaultFaqs: { q: string; a: string }[] = [
    {
      q: `How many characters is a ${name} IBAN?`,
      a: `${country.ibanLength}: "${country.countryCode}", two check digits, then ${country.ibanLength - 4} for the BBAN.`,
    },
    {
      q: `What does a ${name} IBAN look like?`,
      a: `${formatIban(country.exampleIban)} — ${ibanParts.map((part) => `${part.label.toLowerCase()} ${part.value}`).join(", ")}.`,
    },
    {
      q: `Is ${name} part of SEPA?`,
      a: country.sepa
        ? `Yes — ${name} takes SEPA euro transfers${country.currency !== "EUR" ? `; its own currency is ${country.currency}` : ""}.`
        : `No. Payments into ${name} travel over SWIFT, usually in ${country.currency}.`,
    },
    ...(bankNames.length > 0
      ? [{
          q: `Which ${name} banks are listed here?`,
          a: bankNames.length > 3
            ? `${bankNames.slice(0, 3).join(", ")} and ${bankNames.length - 3} more — ${bankNames.length} in all.`
            : `${bankNames.join(", ")}.`,
        }]
      : []),
  ];
    ...
    <Container className="py-8">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          {/* The four stat boxes (length, currency, SEPA, bank count) were
              removed 2026-09-26: the header line and badges state the first
              three and the bank list heads with the fourth. Their label run
              ("IBAN length 22 chars currency EUR SEPA yes") matched every
              country with the same length and currency. */}
          ...
          {/* The "BBAN format details" card was removed 2026-09-26: it listed
              the same fields and lengths as the structure card above, adding
              only validation regexes, and read identically across every
              country sharing a BBAN layout. */}
        ...
        <div className="space-y-6">
          ...
          {/* The "Quick reference" card (country code, length, currency, SEPA,
              example) was removed 2026-09-26: the stat boxes and example card
