...
export interface RateInsight {
   ...
   */
  /**
   * Optional because this file is regenerated by CI on every scrape. A run that
   * checked out a commit predating a new field writes the JSON without it —
   * which is exactly how providerConsistency was silently wiped from all 1,012
   * records hours after shipping. Marking derived fields optional keeps a stale
   * regeneration from breaking the build; consumers already handle absence.
   */
