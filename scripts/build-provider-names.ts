...
// MERGE, don't overwrite. `providerNames` is populated as a side effect of
// importing unified-quotes, and contains ONLY providers present in the CURRENT
// scraped dataset — so a plain write drops any provider absent from the latest
// batch, leaving getProviderName() blank for them until they reappear.
//
// This matters more than it looks: build-provider-names.ts runs in `prebuild`,
// so it executes on every Vercel build, not just after a scrape. A rebuild
// during a partial scrape silently ships a site missing those display names.
// Observed 2026-08-13: a local run dropped `bank-of-america` while adding
// `bbva`.
//
// Union with the existing file so a provider keeps its name once assigned;
// fresh names win on conflict (the scraper is the source of truth for current
// display names).
const existing: Record<string, string> = existsSync(outPath)
  ? (JSON.parse(readFileSync(outPath, "utf8")) as Record<string, string>)
  : {};
